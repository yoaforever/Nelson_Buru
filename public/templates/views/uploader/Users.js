...
angular
    ...
    .controller('AppController', ['$scope', 'FileUploader', '$timeout',function($scope,FileUploader,$timeout) {
		...
		/*uploader.filters.push({
            name: 'csvColumnFilter',
            fn: function(item,options,some) {
                ...
                r.onload = function(e) {
                    ...
                    var dbheader = ["username","name", "lastname", "email", "active","created_at","updated_at","customers_id","connections_id","image_profile","password","remember_token","role_id"];
       ...
       // CALLBACKS
        ...
        uploader.onAfterAddingFile = function(fileItem, item) {
            ...
            r.onload = function(e) {
                ...
                var lines = contents.split('\n');
                var uploadedheader = lines[0].split(",");
                var dbheader = ["customer_id","username", "email_address", "Last_Name", "First_Name","Roll"];
                //console.info('dheader',uploadedheader);
                //console.info('ddbheader',dbheader);
        ...
        uploader.onSuccessItem = function(fileItem, response, status, headers) {
        	...
        	if(response.answer == "Error")
    		{
        		...
        		if($scope.failedcolumns != "")
	        		$scope.failedcolumns += "<br/><br/>"+"Following Users in "+ fileItem.file.name +" are failed to import <br/>"+response.failedrows;
	        	else
	        		$scope.failedcolumns += "Following Users in "+ fileItem.file.name +" are failed to import <br/>"+response.failedrows;
