...
module
    ...
    .factory('FileUploader', ['fileUploaderOptions', '$rootScope', '$http', '$window', '$compile','$location',
        function(fileUploaderOptions, $rootScope, $http, $window, $compile, $location) {
            /**
            ...
            FileUploader.prototype.addToQueue = function(files, options, filters) {
                ...
                angular.forEach(list, function(some,key /*{File|HTMLInputElement|Object}*/) {
                    
					var temp = new FileUploader.FileLikeObject(some);

                    if (this._isValidFile(temp, arrayOfFilters, options,some)) {
                        var fileItem = new FileUploader.FileItem(this, some, options);
                        addedFileItems.push(fileItem);
                        this.queue.push(fileItem);
                        this._onAfterAddingFile(fileItem, some);
                    } else {
			...
			FileUploader.prototype.addChecked = function(value,val) {
				var index = this.getIndexOfItem(value);
                var item = this.queue[index];
				item.isChecked = val;
				console.log(value);
            ...
            FileUploader.prototype.uploadAll = function() {
			 ...
			 FileUploader.prototype.importAll = function() {
				var items = this.getNotUploadedItems().filter(function(item) {
                    return !item.isUploading;
                });
                if (!items.length) return;
                ...
                angular.forEach(items, function(item) {
                    ...
                    if(item.uploader.queue.checkbox.value1)
					{
						//item.url = window.location.origin + '/payanybiz/public/users/import';
						item.url = 'upload.php';
						item._prepareToUploading();
					}else
					{
             ...
             * @private
             */
            FileUploader.prototype._isValidFile = function(file, filters, options,some) {
                this._failFilterIndex = -1;
                return !filters.length ? true : filters.every(function(filter) {
                    this._failFilterIndex++;
					if(filter.name="csvColumnFilter"){
                    	return filter.fn.call(this, file, options,some);
                    }
                    else
            ...
            FileUploader.prototype._xhrTransport = function(item) {
				...
				//xhr.open(item.method, '', true);
                
				//$http.post(item.url, form, {
             ...
             * @param {FileItem} item
             */
            FileUploader.prototype._onAfterAddingFile = function(item,some) {
                this.onAfterAddingFile(item,some);
            };
            /**
             * Inner callback
			...
			FileItem.prototype.checked = function(val) {
				this.uploader.addChecked(this,val);
            };
            ...
            FileItem.prototype._onSuccess = function(response, status, headers) {
				...
				{
					//this.errorMessage = "Error While Importing "; 
					this.isReady = false;
	                this.isUploading = false;
	                this.isUploaded = false;
	                this.isSuccess = false;
