...
//          var_dump($count);
        ...
        foreach ($customer_relation as $relation) {
            foreach ($biller_customer as $payor) {
                if (($relation->customers_id == $payor->id) && ($relation->active == 1)) {
    ...
    function searchBillersConnected($id) {
        $payor_relations = array();
        $customer_relation = CustomerRelation::where('customers1_id', $id)->get();
        $counta = 0;
        $biller_customer = Customer::all();
        ...
        foreach ($customer_relation as $relation) {
            foreach ($biller_customer as $payor) {
                if (($relation->customers_id == $payor->id) && ($payor->id != $id)) {
                    $payor_relations[$counta] = $payor;
                    $counta = $counta + 1;
                }
            }
    ...
    public function searchBillersNotConnected($id) {
        $payor_relations = BillerController::searchBillersConnected($id);
        var_dump($payor_relations);
        $billers_not_connected = array();
        $cont = 0;
        $biller_customer = Customer::all();
        //obtain the biller when the company  invited
        foreach ($payor_relations as $relations) {
            foreach ($biller_customer as $payors) {
                if (($relations->id != $payors->id) && ($payors->id != $id)) {

                    $billers_not_connected[$cont] = $payors;
                    $cont = $cont + 1;
                }
            }
        }
        return $billers_not_connected;
    }
...
//          var_dump($count);
    ...
    public function searchExistBiller() {
        $new = new Customer();
        $legal_name = Input::get('legal_name');

        $bank = new BankAccount;
        $numberd = Input::get('dAccountNumber');
        $numberc = Input::get('cAccountNumber');
        if ($legal_name != '' && $numberd != '' && $numberc != '') {
            $biller = Customer::where('legal_name', $legal_name)->get();
            return $biller;
        }
        if ($numberd != '' && $numberc != '') {

            $bankd = BankAccount::where('number', $numberd)->get();
            $biller = Customer::where('id', $bankd[0]->customers_id)->get();
            return $biller;
        }
        if ($numberc != '') {

            $bankd = BankAccount::where('number', $numberc)->get();
            $biller = Customer::where('id', $bankd[0]->customers_id)->get();
            return $biller;
        } else {
            response()->json(['error' => true, 'not_found' => 1], 400);
        }
    }

    public function connectBillerExist() {
        $billers = new CustomerRelation();
        $billers->customers1_id = Input::get('customer_current');
        $billers->customers_id = Input::get('id_connect');
        $customer_relation = CustomerRelation::where('customers1_id', $billers->customers1_id)->get();
        foreach ($customer_relation as $ralation) {
            if ($ralation->customers_id == $billers->customers_id) {
                return response()->json(['error' => true, 'relation_error' => 1], 400);
            }
        }
        foreach ($customer_relation as $ralations) {
            if ($ralations->customers1_id == $billers->customers_id) {
                return response()->json(['error' => true, 'samebiller_error' => 1], 400);
            }
        }
        $billers->active = 1;
        $billers->save();
        $customer_invite = Customer::where('id', $billers->customers_id)->first();
        $payor = Customer::where('id', $billers->customers1_id)->first();
        $data = array('email' => $customer_invite->email, 'name' => $customer_invite->legal_name, 'payor' => $payor->legal_name);

        Mail::send('emails.connect_biller_exist', array('name' => $data['name'], 'email' => $data['email'], 'payor' => $data['payor']), function($message) use ($data) {
            $message->to($data['email'], $data['name'])->subject('You have been connected as Biller  by ' . $data['payor']);
        });
    ...
    public function updateConnectBiller($id) {
        ...
        $data = array('email' => $payor->email, 'name' => $payor->legal_name, 'biller' => $customer_invite->legal_name);

        Mail::send('emails.connection_biller_accepted', array('name' => $data['name'], 'email' => $data['email'], 'biller' => $data['biller']), function($message) use ($data) {
            $message->to($data['email'], $data['name'])->subject('You have been accepted as Payor  by ' . $data['biller']);
        });
        return Response::json([
                    'success' => true
                        ], 200);
    ...
    public function searchCustomerRelationActive($id) {
//        $payor_relations = array();
        $customer_relation = CustomerRelation::where('customers1_id', $id)->get();
        return $customer_relation;
    }
	
	public function allCustomerRelation() {
       $biller_customer = Customer::all();
        return $biller_customer;
    }
    ...
    public function create() {
        ...
        $exist_biller = Customer::where('email', $new->email)->get();
        if ($exist_customer > 0 && $exist_email > 0) {

            $customer_relation = new CustomerRelation;
            $customer_relation->customers_id = $exist_biller[0]->id;
...
//				return Response::make("Token Expired", 200);
    ...
    public function update($id) {
        ...
        $biller_id = $biller->id;
        $customer_relation = CustomerRelation::where('customers_id', $biller_id)->first();
        $customer_relation->active = 1;
        $customer_relation->save();
        ...
        if ($user->username != null) {
            ...
            $customer_relation = CustomerRelation::where('customers_id', $biller_id)->get();
            $payor_relation = Customer::where('id', $customer_relation[0]->customers1_id)->first();
            $data = array('email' => $biller->email, 'name' => $biller->legal_name, 'payor' => $payor_relation->legal_name);
            Mail::send('emails.biller_without_user', array('name' => $data['name'], 'email' => $data['email'], 'payor' => $data['payor']), function($message) use ($data) {
                $message->to($data['email'], $data['name'])->subject('Welcome to PayAnyBiz!');
            });
